package com.ds.chat_microservice.security;
...
public class JwtService {
    ...
    public boolean authorizeByAdminOrClientRole(String token) {
    ...
    public boolean authorizeByAdminRole(String token) {
        if (checkTokenValidity(token)) {
            String actualToken = token.substring(7);
            Claims claims = extractAllClaims(actualToken);
            String role = claims.get("role").toString();
    ...
    public boolean authorizeByClientRole(String token) {
        if (checkTokenValidity(token)) {
            String actualToken = token.substring(7);
            Claims claims = extractAllClaims(actualToken);
            String role = claims.get("role").toString();
    ...
    public boolean authorizeBasedById(String token, UUID id) {
        if (checkTokenValidity(token)) {
            String actualToken = token.substring(7);
            Claims claims = extractAllClaims(actualToken);
            String role = claims.get("role").toString();
